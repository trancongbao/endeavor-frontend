...
export default function Scheduling({cardId, studyDate, dueDate, showNextCard}) {
...
function schedulingIntervals(studyDate, dueDate) {
    ...
    let interval = (studyDate === null || dueDate <= studyDate)
        ? GRADUATING_INTERVAL
        : Math.floor((new Date(dueDate) - new Date(studyDate)) / (1000 * 60 * 60 * 24));

    return {
        "Again": 0,
        "Easy": Math.floor(interval * EASE_PERCENTAGE * INTERVAL_MODIFIER * EASY_BONUS),
